...
const CONFIG = {
    ...
    OUTPUT_FILE: './Data_Tracking_Processed_Universal_V10.csv',
    CONCURRENT_ROWS: 35, 
...
async function tieredSearchPR(prName, deceasedName, city, state, rowIndex) {
    ...
    // V10: Generating Search Variations (The Sentinel Logic)
    const variations = [prName];
    // Variation 1: Initial only (Vance Monroe Price -> Vance M Price)
    const parts = prName.split(' ');
    if (parts.length > 2) {
        variations.push(`${parts[0]} ${parts[1][0]} ${parts[parts.length-1]}`); // Middle Initial
        variations.push(`${parts[0]} ${parts[parts.length-1]}`); // First Last
    }

    // Attempt Loop
    for (let attempt = 1; attempt <= 2; attempt++) {
        // Try each variation until we find candidates
        let pool = [];
        let chosenName = prName;

        for (const variant of variations) {
            console.log(`[Row ${rowIndex}] 🏎️  Tier 1 Race (Radaris WA + CBC WA) for '${variant}' in ${city}, ${state}...`);
            const [candidatesWA, candidatesCBC] = await Promise.all([
                radaris.search(variant, null, state, { maxPages: 1 }),
                enhanced.searchCBCWithPagination(variant, city, state, { maxPages: 1 })
            ]);
            
            if (candidatesWA.length > 0 || candidatesCBC.length > 0) {
                pool = [...candidatesWA, ...candidatesCBC];
                chosenName = variant;
                break; // Found candidates with this variation
            }
        }
        
        console.log(`[Row ${rowIndex}] Tier 1 Pool: ${pool.length} candidates (used name: ${chosenName}).`);
        ...
        try {
            ...
            // --- TIER 1.5: RECURSIVE RELAY (If we found a relative but no phone) ---
            if (bestMatch && bestMatch.bestMatchIndex !== -1 && bestMatch.confidence >= 60) {
                 const best = pool[bestMatch.bestMatchIndex];
                 if ((!best.visiblePhones || best.visiblePhones.length === 0) && best.relatives && best.relatives.length > 0) {
                     console.log(`[Row ${rowIndex}] 🔗 Recursive Relay: Checking relatives of ${best.fullName} to find PR contact...`);
                     // Pick top 2 relatives
                     const relayTargets = best.relatives.slice(0, 2);
                     for (const relative of relayTargets) {
                         // Search for the relative
                         const [rW, rC] = await Promise.all([
                            radaris.search(relative, null, state, { maxPages: 1 }),
                            enhanced.searchCBCWithPagination(relative, city, state, { maxPages: 1 })
                        ]);
                        const rPool = [...rW, ...rC];
                        // Check if relative's profile lists our PR
                        const backlink = rPool.find(p => p.relatives && p.relatives.some(r => r.includes(parts[0]) && r.includes(parts[parts.length-1])));
                        if (backlink) {
                             const phones = (backlink.visiblePhones || backlink.phones || []).filter(p => p && p.length >= 10);
                             if (phones.length > 0) {
                                 return { 
                                    ...result, 
                                    phone: phones[0], 
                                    allPhones: phones.join(' | '), 
                                    source: `Recursive Relay (via ${relative})`, 
                                    confidence: bestMatch.confidence, // Inherit confidence
                                    reasoning: `Found via household member ${relative}: ${bestMatch.reasoning}`, 
                                    found: true 
                                };
                             }
                        }
            ...
            try {
                 ...
                 const googleQuery = `${chosenName} ${deceasedName} WA`;
...
/**
 * Tier 3: Extract survivors from obituary using Gemini
 */
async function extractSurvivorsFromObituary(deceasedName, state, rowIndex) {
    ...
    try {
        ...
        console.log(`[Row ${rowIndex}] Obituary extraction error: ${e.message}`);
        return [];
    }
}

/**
 * Tier 3: Address-centric resident search
 */
async function addressCentricSearch(address, deceasedName, rowIndex) {
    if (!address) return [];
    try {
        ...
        items.forEach(item => {
            ...
            if (nameMatch && !item.title.includes('Zillow') && !item.title.includes('Redfin')) {
                candidates.push({
                    fullName: nameMatch[1].trim(),
                    phones: snippetPhones,
                    source: 'Address Pivot',
                    detailLink: item.link,
                    snippet: item.snippet
                });
            }
...
async function processRow(row, rowIndex) {
    ...
    // --- V9 TIER 3: DEEP EXCAVATION ---
    if (prResults.every(res => !res.found) && parsed.deceased_name) {
        console.log(`[Row ${rowIndex}] 🕵️  Tier 3 Deep Excavation (Pivoting)...`);
        
        // 1. Obituary Survivor Pivot
        const survivors = await extractSurvivorsFromObituary(parsed.deceased_name, state, rowIndex);
        if (survivors.length > 0) {
            console.log(`[Row ${rowIndex}] 🕯️  Survivors found: ${survivors.slice(0,3).join(', ')}`);
            ...
            if (successfulSurvivor) {
                prResults[0] = { ...successfulSurvivor, reasoning: `Recovered via Obituary Analysis: ${successfulSurvivor.reasoning}` };
            }
        }

        // 2. Address Pivot (if still nothing)
        if (prResults.every(res => !res.found) && address) {
            console.log(`[Row ${rowIndex}] 🏠 Address Centric Pivot...`);
            const addrCandidates = await addressCentricSearch(address, parsed.deceased_name, rowIndex);
            if (addrCandidates.length > 0) {
                const targetInfo = `${city || ''} ${state || ''}`.trim();
                const bestAddrMatch = await matchProfile(parsed.deceased_name, targetInfo, "Resident", addrCandidates);
                if (bestAddrMatch && bestAddrMatch.bestMatchIndex !== -1 && bestAddrMatch.confidence >= 50) {
                    ...
                    if (best.phones && best.phones.length > 0) {
                        prResults[0] = { 
                            name: best.fullName, 
                            phone: best.phones[0], 
                            allPhones: best.phones.join(' | '), 
                            source: 'Address Pivot (Tier 3)', 
                            confidence: bestAddrMatch.confidence, 
                            reasoning: `Found living at target property: ${bestAddrMatch.reasoning}`, 
                            found: true 
                        };
                    }
                }
...
async function main() {
    ...
    console.log('UNIVERSAL SCRAPER V10: THE SENTINEL (PRODUCTION READY)');
    console.log('='.repeat(60));
