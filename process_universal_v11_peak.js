...
async function tieredSearchPR(prName, deceasedName, city, state, rowIndex) {
    ...
    // Radaris strips middle names internally, so only send First+Last once (avoids duplicate calls)
    const radarisName = parts.length > 2 ? `${parts[0]} ${parts[parts.length-1]}` : prName;
    const searches = [
        radaris.search(radarisName, null, state, { maxPages: 1 }).catch(e => []),
        // CBC benefits from all variations (middle names, hyphenated surnames)
        ...variations.map(v => enhanced.searchCBCWithPagination(v, city, state, { maxPages: 1 }).catch(e => []))
    ];
    ...
    if (hasPerfect) {
        console.log(`[Row ${rowIndex}] ΓÜí Perfect Tier 1 match found. Speed-bypassing Tier 2.`);
    } else {
    ...
    // AI MATCHING
    console.log(`[Row ${rowIndex}] ≡ƒñû AI Matching ${masterPool.length} candidates...`);
    ...
    if (match && match.bestMatchIndex !== -1 && match.confidence >= CONFIG.CONFIDENCE_THRESHOLD_MEDIUM) {
        ...
        if (best.relatives && best.relatives.length > 0) {
            console.log(`[Row ${rowIndex}] ≡ƒöù Relay: ${best.fullName}...`);
...
async function addressCentricSearch(address, rowIndex) {
    ...
    try {
        ...
        return items.map(item => {
            const snippetPhones = (item.snippet.match(/\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}/g) || []);
            const nameMatch = item.title.match(/^([^|ΓÇó-]+)/);
...
async function processRow(row, rowIndex) {
    ...
    if (prResults.every(res => !res.found) && parsed.deceased_name) {
        console.log(`[Row ${rowIndex}] ≡ƒò╡∩╕Å  V11 Deep Excavation...`);
        ...
        if (prResults.every(res => !res.found) && prList[0]) {
            console.log(`[Row ${rowIndex}] ≡ƒô₧ Tier 4: WhitePages Fallback...`);
            ...
            for (let attempt = 1; attempt <= 2; attempt++) {
                try {
                    ...
                    if (wpResult && wpResult.phones && wpResult.phones.length > 0) {
                        ...
                        console.log(`[Row ${rowIndex}] Γ£à WhitePages found: ${wpResult.phones[0]}`);
                        break; // Success, exit retry loop
                    } else if (attempt < 2) {
                        console.log(`[Row ${rowIndex}] ΓÅ│ WhitePages attempt ${attempt} returned no phones, retrying...`);
                        await new Promise(r => setTimeout(r, 5000));
                    }
                } catch (e) {
                    console.log(`[Row ${rowIndex}] ΓÜá∩╕Å WhitePages attempt ${attempt} error: ${e.message}`);
                    if (attempt < 2) {
                        await new Promise(r => setTimeout(r, 5000));
                    }
                }
